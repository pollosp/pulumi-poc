...
pulumi-update: 
	docker run -ti -e PULUMI_ACCESS_TOKEN="$$PULUMI_ACCESS_TOKEN" -e GOOGLE_CREDENTIALS="$$CREDENTIALS" $(CONTAINER_GCP_NAME) update -s $(STACK)
pulumi-preview: 
	docker run -ti -e PULUMI_ACCESS_TOKEN="$$PULUMI_ACCESS_TOKEN" -e GOOGLE_CREDENTIALS="$$CREDENTIALS" $(CONTAINER_GCP_NAME) preview -s $(STACK)
#Due a bug using ENV vars described in https://github.com/pulumi/pulumi-gcp/issues/25
config:
